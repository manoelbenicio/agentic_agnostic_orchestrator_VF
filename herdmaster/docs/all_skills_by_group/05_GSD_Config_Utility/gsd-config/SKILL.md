...
<execution_context>
@~/.gemini/antigravity/gsd-core/workflows/settings.md
@~/.gemini/antigravity/gsd-core/workflows/settings-advanced.md
@~/.gemini/antigravity/gsd-core/workflows/settings-integrations.md
</execution_context>
...
- If it starts with `--profile`: extract the profile name (remainder after `--profile`), then:
  1. **Pre-flight check (#2439):** verify `gsd-sdk` is on PATH via `command -v gsd-sdk`.
     If absent, emit the install hint `Install GSD via 'npm i -g gsd-core'` and stop —
