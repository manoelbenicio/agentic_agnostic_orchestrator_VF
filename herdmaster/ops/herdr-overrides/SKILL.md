...
> NOTE (local fleet): this Herdr 0.7.x session uses `w<N>:p<X>` style ids (e.g. `w8:pQ`) — the
> `1-1` examples below are illustrative. The rule below still holds: always re-read ids from
> `pane list` / `workspace list` / split responses; never hardcode.
...
> NOTE (local finding 2026-06-27): for screen-manifest agents (codex/agy/kiro here),
> `wait agent-status --status idle` proved UNRELIABLE (timed out across a real working→idle
> transition). For "settle to idle", poll `agent explain <pane> --json` instead. `wait agent-status
> --status working` and `--status done` work for transitions.
